...
function clearCatalogContent() {
  ...
  const topScrollInner = document.getElementById('catalogTopScrollInner');
  const topScroll = document.getElementById('catalogTopScroll');
  const tableWrap = document.getElementById('catalogTableWrap');
  if (head) head.innerHTML = '';
  if (body) body.innerHTML = '';
  if (filterBar) filterBar.innerHTML = '';
  if (meta) meta.textContent = 'Cargando cursos...';
  if (topScrollInner) topScrollInner.style.width = '0px';
...
function syncCatalogScrollWidth() {
...
function initCatalogScrollSync() {
  const topScroll = document.getElementById('catalogTopScroll');
  const tableWrap = document.getElementById('catalogTableWrap');
  if (!topScroll || !tableWrap) return;
  ...
  window.addEventListener('resize', syncCatalogScrollWidth);
}
